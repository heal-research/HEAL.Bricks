...
namespace HEAL.Bricks {
  ...
  internal sealed class LightweightApplicationManager : IApplicationManager {
    ...
    Assembly CurrentDomain_AssemblyResolve(object sender, ResolveEventArgs args) => null;
    ...
    /// </summary>
    public IEnumerable<IPluginDescription> Plugins => new IPluginDescription[0];

    /// <summary>
    /// Gets an empty list of applications. (LightweightApplicationManager doesn't support application discovery)
    /// </summary>
    public IEnumerable<IApplicationDescription> Applications => new IApplicationDescription[0];
    ...
    public IEnumerable<T> GetInstances<T>() where T : class => GetInstances(typeof(T)).Cast<T>();
    ...
    public IEnumerable<object> GetInstances(Type type, params object[] args) {
      ...
      foreach (Type t in GetTypes(type)) {
        ...
        try { instance = Activator.CreateInstance(t, args); } catch { }
    ...
    /// <summary>
    /// Discovers a specific type by its name.
    /// </summary>
    /// <param name="typeName">Full name of the type.</param>
    /// <returns>A type or null, if nothing was found.</returns>

  ...
  public class MessageHandler : IMessageHandler {
    ...
    public async Task ReceiveMessagesAsync(IChannel channel, CancellationToken cancellationToken = default) {
      ...
      try {
        while (!cts.Token.IsCancellationRequested) {
          IMessage message = await channel.ReceiveMessageAsync(cts.Token);
    ...
    public static class DefaultHandlers {
      public static async Task LoadPackagesAsync(PackageLoadInfo[] packages, IChannel channel, CancellationToken cancellationToken) {
        ...
        await channel.SendMessageAsync(Message.Create(Message.Commands.PackagesLoaded), cancellationToken);
      }
      ...
      public static async Task DiscoverRunnablesAsync(IChannel channel, CancellationToken cancellationToken) {
        ...
        await channel.SendMessageAsync(Message.Create(Message.Commands.RunnablesDiscovered, runnables), cancellationToken);
      }
      ...
      public static async Task RunRunnableAsync(Tuple<RunnableInfo, string[]> info, IChannel channel, CancellationTokenSource cts) {
        ...
        IRunnable runnable = runnableInfo.CreateRunnable(channel);
        switch (runnable) {
          case IApplication application:
            Task appTask = Task.Run(async () => {
              await application.RunAsync(args, cts.Token);
              cts.Cancel();
            });
            if (application.Kind == ApplicationKind.Console) {
              await appTask;
            }
            break;
          case IService service:
            await service.StartAsync(args, cts.Token);
            break;
        }
      }
